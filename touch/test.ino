...
// TFT Touch and Display
TFT_eSPI tft = TFT_eSPI();
uint16_t calData[5] = {275, 3590, 285, 3541, 7}; // Update with your calibration values
...
// ====== TFT Touch Functions ======
void drawButtons() {
  for (int i = 0; i < 4; i++) updateButton(i);
}
...
int getButton(int x, int y) {
  for (int i = 0; i < 4; i++) {
    ...
    if (x > bx && x < bx + BUTTON_W && y > by && y < by + BUTTON_H) return i;
  }
  return -1;
}

// ====== Web and Device Functions ======
...
void handleAddDevice() {
  if (deviceCount >= MAX_DEVICES) return server.send(200, "text/plain", "Limit reached");
  String name = server.arg("name");
  int gpio = server.arg("gpio").toInt();
  if (name == "" || gpio < 0 || gpio > 39) return server.send(200, "text/plain", "Invalid");
  devices[deviceCount] = { name, gpio, false };
  pinMode(gpio, OUTPUT);
...
void startWebServer(); // forward declaration
void handleRoot() {
...
void setup() {
  ...
  if (wifiConnected && checkForUpdate()) downloadFirmware(firmwareURL);
  startWebServer();
